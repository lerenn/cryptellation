...
func (wf *workflows) GetExchangeWorkflow(
	...
	err := workflow.ExecuteActivity(ctx, wf.db.ReadExchangesActivity, db.ReadExchangesActivityParams{
	...
	err = workflow.ExecuteActivity(ctx, wf.exchanges.GetExchangeActivity, exchangesactivity.GetExchangeActivityParams{
		Name: params.Name,
	}).Get(ctx, &r)
	if err != nil {
	...
	if len(dbRes.Exchanges) == 0 {
		if err := workflow.ExecuteActivity(ctx, wf.db.CreateExchangesActivity, db.CreateExchangesActivityParams{
			Exchanges: []exchange.Exchange{r.Exchange},
		}).Get(ctx, nil); err != nil {
			return api.GetExchangeWorkflowResults{}, err
		}
	} else {
		if err := workflow.ExecuteActivity(ctx, wf.db.UpdateExchangesActivity, db.UpdateExchangesActivityParams{
			Exchanges: []exchange.Exchange{r.Exchange},
		}).Get(ctx, nil); err != nil {
			return api.GetExchangeWorkflowResults{}, err
		}
