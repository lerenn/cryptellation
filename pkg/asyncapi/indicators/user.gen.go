...
type UserSubscriber interface {
	// CryptellationIndicatorsSmaResponse subscribes to messages placed on the 'cryptellation.indicators.sma.response' channel
	CryptellationIndicatorsSmaResponse(ctx context.Context, msg SmaResponseMessage)
}
...
func (c *UserController) SubscribeCryptellationIndicatorsSmaResponse(ctx context.Context, fn func(ctx context.Context, msg SmaResponseMessage)) error {
	// Get channel path
	...
	go func() {
		for {
			...
			if err := c.executeMiddlewares(ctx, &brokerMsg, func(ctx context.Context) error {
				// Process message
				msg, err := newSmaResponseMessageFromBrokerMessage(brokerMsg)
				if err != nil {
					return err
				}
...
func (c *UserController) UnsubscribeCryptellationIndicatorsSmaResponse(ctx context.Context) {
	// Get channel path
	path := "cryptellation.indicators.sma.response"
...
func (c *UserController) PublishCryptellationIndicatorsSmaRequest(ctx context.Context, msg SmaRequestMessage) error {
	// Get channel path
...
//
// A timeout can be set in context to avoid blocking operation, if needed.
func (cc *UserController) WaitForCryptellationIndicatorsSmaResponse(ctx context.Context, publishMsg MessageWithCorrelationID, pub func(ctx context.Context) error) (SmaResponseMessage, error) {
	// Get channel path
	path := "cryptellation.indicators.sma.response"
	...
	for {
		...
		case brokerMsg, open := <-sub.MessagesChannel():
			...
			if !open && brokerMsg.IsUninitialized() {
				...
				return SmaResponseMessage{}, extensions.ErrSubscriptionCanceled
			}
			...
			return newSmaResponseMessageFromBrokerMessage(brokerMsg)
		case <-ctx.Done(): // Set corrsponding error if context is done
			cc.logger.Error(ctx, "Context done before getting message")
			return SmaResponseMessage{}, extensions.ErrContextCanceled
		}
	}
}
