...
func (suite *BacktestsSuite) checkTickEvent(expectedTime time.Time, expectedTick tick.Tick, receivedEvt event.Event) {
	// Check type and time
...
func (suite *BacktestsSuite) checkStatusEvent(expectedTime time.Time, isFinished bool, receivedEvt event.Event) {
	// Check type and time
...
func (suite *BacktestsSuite) TestBacktestOrder() {
	// Create backtest
	id, err := suite.client.Create(context.Background(), client.BacktestCreationPayload{
		Accounts: map[string]account.Account{
			"binance": {
