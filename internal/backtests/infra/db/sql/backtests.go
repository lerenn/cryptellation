...
func (sqldb *DB) CreateBacktest(ctx context.Context, bt *domain.Backtest) error {
	var count int64
...
func (sqldb *DB) readBacktest(ctx context.Context, id uint, clauses []clause.Expression) (domain.Backtest, error) {
	e := entities.Backtest{
		ID: id,
	}
	...
	tx := sqldb.client.
		...
		Find(&e)
	if tx.Error != nil {
		return domain.Backtest{}, tx.Error
	} else if tx.RowsAffected == 0 {
		return domain.Backtest{}, db.ErrRecordNotFound
	}

	return e.ToModel()
}

func (sqldb *DB) ReadBacktest(ctx context.Context, id uint) (domain.Backtest, error) {
	return sqldb.readBacktest(ctx, id, nil)
}

func (sqldb *DB) UpdateBacktest(ctx context.Context, bt domain.Backtest) error {
	entity := entities.FromBacktestModel(bt)
...
func (sqldb *DB) DeleteBacktest(ctx context.Context, bt domain.Backtest) error {
	entity := entities.FromBacktestModel(bt)
