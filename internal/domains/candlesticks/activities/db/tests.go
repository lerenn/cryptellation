...
func (suite *CandlesticksSuite) TestCreate() {
	...
	_, err := suite.DB.CreateCandlesticks(context.Background(), CreateCandlesticksParams{
		List: list,
	})
	suite.Require().NoError(err)
	res, err := suite.DB.ReadCandlesticks(context.Background(), ReadCandlesticksParams{
		Exchange: "exchange",
		Pair:     "ETH-USDC",
		Period:   period.M1,
...
func (suite *CandlesticksSuite) TestUpdate() {
	...
	_, err = suite.DB.UpdateCandlesticks(context.Background(), UpdateCandlesticksParams{
		List: update,
	})
	suite.Require().NoError(err)
	res, err := suite.DB.ReadCandlesticks(context.Background(), ReadCandlesticksParams{
...
func (suite *CandlesticksSuite) TestDelete() {
	...
	_, err = suite.DB.DeleteCandlesticks(context.Background(), DeleteCandlesticksParams{
		List: halfList,
	})
	suite.Require().NoError(err)
