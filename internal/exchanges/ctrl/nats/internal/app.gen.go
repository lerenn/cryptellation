...
type AppSubscriber interface {
	...
	ExchangesListRequest(msg ExchangesRequestMessage)
}
...
func (c *AppController) SubscribeExchangesListRequest(fn func(msg ExchangesRequestMessage)) error {
	// Check if there is already a subscription
	_, exists := c.stopSubscribers["exchanges.list.request"]
	if exists {
		return fmt.Errorf("%w: exchanges.list.request channel is already subscribed", ErrAlreadySubscribedChannel)
	}

	// Subscribe to broker channel
	msgs, stop, err := c.brokerController.Subscribe("exchanges.list.request")
	if err != nil {
		return err
	}
	...
	go func() {
		for um, open := <-msgs; open; um, open = <-msgs {
			...
			if err != nil {
				c.handleError("exchanges.list.request", err)
			} else {
				fn(msg)
			}
		}
	...
	c.stopSubscribers["exchanges.list.request"] = stop
...
func (c *AppController) UnsubscribeExchangesListRequest() {
	stopChan, exists := c.stopSubscribers["exchanges.list.request"]
	if !exists {
		return
	}

	stopChan <- true
	delete(c.stopSubscribers, "exchanges.list.request")
}
...
func (c *AppController) PublishExchangesListResponse(msg ExchangesResponseMessage) error {
	...
	return c.brokerController.Publish("exchanges.list.response", um)
}
