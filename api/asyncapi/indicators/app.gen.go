...
type AppSubscriber interface {
	// ServiceInfoRequest subscribes to messages placed on the 'cryptellation.indicators.service.info.request' channel
	ServiceInfoRequest(ctx context.Context, msg ServiceInfoRequestMessage)

	// GetSMARequest subscribes to messages placed on the 'cryptellation.indicators.sma.request' channel
	GetSMARequest(ctx context.Context, msg GetSMARequestMessage)
}
...
func (c *AppController) SubscribeAll(ctx context.Context, as AppSubscriber) error {
	...
	if err := c.SubscribeGetSMARequest(ctx, as.GetSMARequest); err != nil {
		return err
	}
...
func (c *AppController) UnsubscribeAll(ctx context.Context) {
	...
	c.UnsubscribeGetSMARequest(ctx)
}
...
// UnsubscribeServiceInfoRequest will unsubscribe messages from 'cryptellation.indicators.service.info.request' channel.
...
} // SubscribeGetSMARequest will subscribe to new messages from 'cryptellation.indicators.sma.request' channel.
// Callback function 'fn' will be called each time a new message is received.
func (c *AppController) SubscribeGetSMARequest(ctx context.Context, fn func(ctx context.Context, msg GetSMARequestMessage)) error {
	...
	go func() {
		for {
			...
			if err := c.executeMiddlewares(ctx, &brokerMsg, func(ctx context.Context) error {
				// Process message
				msg, err := newGetSMARequestMessageFromBrokerMessage(brokerMsg)
				if err != nil {
					return err
				}
...
// UnsubscribeGetSMARequest will unsubscribe messages from 'cryptellation.indicators.sma.request' channel.
// A timeout can be set in context to avoid blocking operation, if needed.
