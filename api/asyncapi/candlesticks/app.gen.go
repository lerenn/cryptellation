...
func (c *AppController) SubscribeAll(ctx context.Context, as AppSubscriber) error {
	...
	if err := c.SubscribeListCandlesticksRequest(ctx, as.ListCandlesticksRequest); err != nil {
		return err
	}
...
func (c *AppController) UnsubscribeAll(ctx context.Context) {
	c.UnsubscribeListCandlesticksRequest(ctx)
	...
	go func() {
		for {
			...
			if err := c.executeMiddlewares(ctx, &brokerMsg, func(ctx context.Context) error {
				// Process message
				msg, err := newListCandlesticksRequestMessageFromBrokerMessage(brokerMsg)
				if err != nil {
					return err
				}
...
// UnsubscribeListCandlesticksRequest will unsubscribe messages from 'cryptellation.candlesticks.list.request' channel.
...
} // SubscribeServiceInfoRequest will subscribe to new messages from 'cryptellation.candlesticks.service.info.request' channel.
// Callback function 'fn' will be called each time a new message is received.
...
// UnsubscribeServiceInfoRequest will unsubscribe messages from 'cryptellation.candlesticks.service.info.request' channel.
// A timeout can be set in context to avoid blocking operation, if needed.
