	...
	go func() {
		for {
			...
			if err := c.executeMiddlewares(ctx, &acknowledgeableBrokerMessage.BrokerMessage, func(ctx context.Context) error {
				// Process message
				msg, err := newEventMessageFromBrokerMessage(acknowledgeableBrokerMessage.BrokerMessage)
				if err != nil {
					return err
				}
...
func (c *UserController) RequestToAccountsListOperation(
	...
	for {
		...
		case acknowledgeableBrokerMessage, open := <-sub.MessagesChannel():
			...
			// Get new message
			rmsg, err := newAccountsListResponseMessageFromBrokerMessage(acknowledgeableBrokerMessage.BrokerMessage)
			if err != nil {
				c.logger.Error(ctx, err.Error())
			}
			...
			return newAccountsListResponseMessageFromBrokerMessage(acknowledgeableBrokerMessage.BrokerMessage)
...
func (c *UserController) RequestToAdvanceOperation(
	...
	for {
		...
		case acknowledgeableBrokerMessage, open := <-sub.MessagesChannel():
			...
			// Get new message
			rmsg, err := newAdvanceResponseMessageFromBrokerMessage(acknowledgeableBrokerMessage.BrokerMessage)
			if err != nil {
				c.logger.Error(ctx, err.Error())
			}
			...
			// NOTE: it is transformed from the broker again, as it could have
			// been modified by middlewares
			return newAdvanceResponseMessageFromBrokerMessage(acknowledgeableBrokerMessage.BrokerMessage)
		case <-ctx.Done(): // Set corrsponding error if context is done
			c.logger.Error(ctx, "Context done before getting message")
...
func (c *UserController) RequestToCreateOperation(
	...
	for {
		...
		case acknowledgeableBrokerMessage, open := <-sub.MessagesChannel():
			...
			// Get new message
			rmsg, err := newCreateResponseMessageFromBrokerMessage(acknowledgeableBrokerMessage.BrokerMessage)
			if err != nil {
				c.logger.Error(ctx, err.Error())
			}
			...
			// NOTE: it is transformed from the broker again, as it could have
			// been modified by middlewares
			return newCreateResponseMessageFromBrokerMessage(acknowledgeableBrokerMessage.BrokerMessage)
		case <-ctx.Done(): // Set corrsponding error if context is done
			c.logger.Error(ctx, "Context done before getting message")
...
func (c *UserController) RequestToOrdersCreateOperation(
	...
	for {
		...
		case acknowledgeableBrokerMessage, open := <-sub.MessagesChannel():
			...
			// Get new message
			rmsg, err := newOrdersCreateResponseMessageFromBrokerMessage(acknowledgeableBrokerMessage.BrokerMessage)
			if err != nil {
				c.logger.Error(ctx, err.Error())
			}
			...
			// NOTE: it is transformed from the broker again, as it could have
			// been modified by middlewares
			return newOrdersCreateResponseMessageFromBrokerMessage(acknowledgeableBrokerMessage.BrokerMessage)
		case <-ctx.Done(): // Set corrsponding error if context is done
			c.logger.Error(ctx, "Context done before getting message")
...
func (c *UserController) RequestToOrdersListOperation(
	...
	for {
		...
		case acknowledgeableBrokerMessage, open := <-sub.MessagesChannel():
			...
			// Get new message
			rmsg, err := newOrdersListResponseMessageFromBrokerMessage(acknowledgeableBrokerMessage.BrokerMessage)
			if err != nil {
				c.logger.Error(ctx, err.Error())
			}
			...
			// NOTE: it is transformed from the broker again, as it could have
			// been modified by middlewares
			return newOrdersListResponseMessageFromBrokerMessage(acknowledgeableBrokerMessage.BrokerMessage)
		case <-ctx.Done(): // Set corrsponding error if context is done
			c.logger.Error(ctx, "Context done before getting message")
...
func (c *UserController) RequestToSubscribeOperation(
	...
	for {
		...
		case acknowledgeableBrokerMessage, open := <-sub.MessagesChannel():
			...
			// Get new message
			rmsg, err := newSubscribeResponseMessageFromBrokerMessage(acknowledgeableBrokerMessage.BrokerMessage)
			if err != nil {
				c.logger.Error(ctx, err.Error())
			}
			...
			// NOTE: it is transformed from the broker again, as it could have
			// been modified by middlewares
			return newSubscribeResponseMessageFromBrokerMessage(acknowledgeableBrokerMessage.BrokerMessage)
		case <-ctx.Done(): // Set corrsponding error if context is done
			c.logger.Error(ctx, "Context done before getting message")
