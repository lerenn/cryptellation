...
type UserSubscriber interface {
	// ServiceInfoResponse subscribes to messages placed on the 'cryptellation.indicators.service.info.response' channel
	ServiceInfoResponse(ctx context.Context, msg ServiceInfoResponseMessage)

	// GetSMAResponse subscribes to messages placed on the 'cryptellation.indicators.sma.response' channel
	GetSMAResponse(ctx context.Context, msg GetSMAResponseMessage)
}
...
func (c *UserController) SubscribeAll(ctx context.Context, as UserSubscriber) error {
	...
	if err := c.SubscribeGetSMAResponse(ctx, as.GetSMAResponse); err != nil {
		return err
	}
...
func (c *UserController) UnsubscribeAll(ctx context.Context) {
	...
	c.UnsubscribeGetSMAResponse(ctx)
}
...
func (c *UserController) SubscribeServiceInfoResponse(ctx context.Context, fn func(ctx context.Context, msg ServiceInfoResponseMessage)) error {
	// Get channel path
	path := "cryptellation.indicators.service.info.response"
...
// UnsubscribeServiceInfoResponse will unsubscribe messages from 'cryptellation.indicators.service.info.response' channel.
...
} // SubscribeGetSMAResponse will subscribe to new messages from 'cryptellation.indicators.sma.response' channel.
// Callback function 'fn' will be called each time a new message is received.
func (c *UserController) SubscribeGetSMAResponse(ctx context.Context, fn func(ctx context.Context, msg GetSMAResponseMessage)) error {
	...
	go func() {
		for {
			...
			if err := c.executeMiddlewares(ctx, &brokerMsg, func(ctx context.Context) error {
				// Process message
				msg, err := newGetSMAResponseMessageFromBrokerMessage(brokerMsg)
				if err != nil {
					return err
				}
...
// UnsubscribeGetSMAResponse will unsubscribe messages from 'cryptellation.indicators.sma.response' channel.
// A timeout can be set in context to avoid blocking operation, if needed.
...
func (c *UserController) PublishServiceInfoRequest(ctx context.Context, msg ServiceInfoRequestMessage) error {
	// Get channel path
	path := "cryptellation.indicators.service.info.request"
...
func (c *UserController) WaitForGetSMAResponse(ctx context.Context, publishMsg MessageWithCorrelationID, pub func(ctx context.Context) error) (GetSMAResponseMessage, error) {
	...
	if err != nil {
		c.logger.Error(ctx, err.Error())
		return GetSMAResponseMessage{}, err
	}
	c.logger.Info(ctx, "Subscribed to channel")
	...
	for {
		...
		case brokerMsg, open := <-sub.MessagesChannel():
			...
			if !open && brokerMsg.IsUninitialized() {
				c.logger.Error(ctx, "Channel closed before getting message")
				return GetSMAResponseMessage{}, extensions.ErrSubscriptionCanceled
			}
			...
			// Return the message to the caller from the broker that could have
			// been modified by middlewares
			return newGetSMAResponseMessageFromBrokerMessage(brokerMsg)
		case <-ctx.Done(): // Set corrsponding error if context is done
			c.logger.Error(ctx, "Context done before getting message")
			return GetSMAResponseMessage{}, extensions.ErrContextCanceled
		}
	}
}
