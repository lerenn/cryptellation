...
func (c *UserController) RequestToSMAOperation(
	...
	for {
		...
		case acknowledgeableBrokerMessage, open := <-sub.MessagesChannel():
			...
			// Get new message
			rmsg, err := brokerMessageToSMAResponseMessage(acknowledgeableBrokerMessage.BrokerMessage)
			if err != nil {
				c.logger.Error(ctx, err.Error())
			}
			...
			return brokerMessageToSMAResponseMessage(acknowledgeableBrokerMessage.BrokerMessage)
...
func (c *UserController) SendToServiceInfoOperation(
	...
	// Set channel address
	addr := "cryptellation.indicators.service.info"
