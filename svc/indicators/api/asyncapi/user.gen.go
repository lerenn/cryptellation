...
// SendToSMAOperation will send a SMARequest message on SMARequest channel.
//
...
func (c *UserController) SendToSMAOperation(
	ctx context.Context,
	msg SMARequestMessage,
) error {
	// Set channel address
...
// RequestToSMAOperation will send a SMARequest message on SMARequest channel
// and wait for a SMAResponse message from SMAResponse channel.
//
...
func (c *UserController) RequestToSMAOperation(
	ctx context.Context,
	msg SMARequestMessage,
) (SMAResponseMessage, error) {
	...
	// Send the message
	if err := c.SendToSMAOperation(ctx, msg); err != nil {
		c.logger.Error(ctx, "error happened when sending message", extensions.LogInfo{Key: "error", Value: err.Error()})
		return SMAResponseMessage{}, fmt.Errorf("error happened when sending message: %w", err)
	}
	...
	for {
		...
		case acknowledgeableBrokerMessage, open := <-sub.MessagesChannel():
			...
			// Get new message
			rmsg, err := newSMAResponseMessageFromBrokerMessage(acknowledgeableBrokerMessage.BrokerMessage)
			if err != nil {
				c.logger.Error(ctx, err.Error())
			}
			...
			return newSMAResponseMessageFromBrokerMessage(acknowledgeableBrokerMessage.BrokerMessage)
