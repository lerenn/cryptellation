...
func (c *UserController) SubscribeAll(ctx context.Context, as UserSubscriber) error {
	...
	if err := c.SubscribeListCandlesticksResponse(ctx, as.ListCandlesticksResponse); err != nil {
		return err
	}
...
func (c *UserController) UnsubscribeAll(ctx context.Context) {
	c.UnsubscribeListCandlesticksResponse(ctx)
	...
	go func() {
		for {
			...
			if err := c.executeMiddlewares(ctx, &brokerMsg, func(ctx context.Context) error {
				// Process message
				msg, err := newListCandlesticksResponseMessageFromBrokerMessage(brokerMsg)
				if err != nil {
					return err
				}
...
// UnsubscribeListCandlesticksResponse will unsubscribe messages from 'cryptellation.candlesticks.list.response' channel.
...
func (c *UserController) PublishServiceInfoRequest(ctx context.Context, msg ServiceInfoRequestMessage) error {
	// Get channel path
	path := "cryptellation.candlesticks.service.info.request"
...
func (c *UserController) WaitForListCandlesticksResponse(ctx context.Context, publishMsg MessageWithCorrelationID, pub func(ctx context.Context) error) (ListCandlesticksResponseMessage, error) {
	...
	if err != nil {
		c.logger.Error(ctx, err.Error())
		return ListCandlesticksResponseMessage{}, err
	}
	c.logger.Info(ctx, "Subscribed to channel")
	...
	for {
		...
		case brokerMsg, open := <-sub.MessagesChannel():
			...
			if !open && brokerMsg.IsUninitialized() {
				...
				return ListCandlesticksResponseMessage{}, extensions.ErrSubscriptionCanceled
			}
			...
			return newListCandlesticksResponseMessageFromBrokerMessage(brokerMsg)
		case <-ctx.Done(): // Set corrsponding error if context is done
			c.logger.Error(ctx, "Context done before getting message")
			return ListCandlesticksResponseMessage{}, extensions.ErrContextCanceled
		}
	}
}
