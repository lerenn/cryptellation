...
type UserSubscriber interface {
	// ListExchangesResponse subscribes to messages placed on the 'cryptellation.exchanges.list.response' channel
	ListExchangesResponse(ctx context.Context, msg ListExchangesResponseMessage)

	// ServiceInfoResponse subscribes to messages placed on the 'cryptellation.exchanges.service.info.response' channel
...
func (c *UserController) SubscribeAll(ctx context.Context, as UserSubscriber) error {
	...
	if err := c.SubscribeListExchangesResponse(ctx, as.ListExchangesResponse); err != nil {
		return err
	}
...
func (c *UserController) UnsubscribeAll(ctx context.Context) {
	c.UnsubscribeListExchangesResponse(ctx)
	c.UnsubscribeServiceInfoResponse(ctx)
}

// SubscribeListExchangesResponse will subscribe to new messages from 'cryptellation.exchanges.list.response' channel.
//
// Callback function 'fn' will be called each time a new message is received.
func (c *UserController) SubscribeListExchangesResponse(ctx context.Context, fn func(ctx context.Context, msg ListExchangesResponseMessage)) error {
	// Get channel path
	...
	go func() {
		for {
			...
			if err := c.executeMiddlewares(ctx, &brokerMsg, func(ctx context.Context) error {
				// Process message
				msg, err := newListExchangesResponseMessageFromBrokerMessage(brokerMsg)
				if err != nil {
					return err
				}
...
// UnsubscribeListExchangesResponse will unsubscribe messages from 'cryptellation.exchanges.list.response' channel.
// A timeout can be set in context to avoid blocking operation, if needed.
func (c *UserController) UnsubscribeListExchangesResponse(ctx context.Context) {
	// Get channel path
	path := "cryptellation.exchanges.list.response"
...
} // SubscribeServiceInfoResponse will subscribe to new messages from 'cryptellation.exchanges.service.info.response' channel.
// Callback function 'fn' will be called each time a new message is received.
...
// UnsubscribeServiceInfoResponse will unsubscribe messages from 'cryptellation.exchanges.service.info.response' channel.
// A timeout can be set in context to avoid blocking operation, if needed.
...
// PublishListExchangesRequest will publish messages to 'cryptellation.exchanges.list.request' channel
func (c *UserController) PublishListExchangesRequest(ctx context.Context, msg ListExchangesRequestMessage) error {
	// Get channel path
...
// PublishServiceInfoRequest will publish messages to 'cryptellation.exchanges.service.info.request' channel
...
// WaitForListExchangesResponse will wait for a specific message by its correlation ID.
//
...
//
// A timeout can be set in context to avoid blocking operation, if needed.
func (c *UserController) WaitForListExchangesResponse(ctx context.Context, publishMsg MessageWithCorrelationID, pub func(ctx context.Context) error) (ListExchangesResponseMessage, error) {
	// Get channel path
	path := "cryptellation.exchanges.list.response"
	...
	if err != nil {
		c.logger.Error(ctx, err.Error())
		return ListExchangesResponseMessage{}, err
	}
	c.logger.Info(ctx, "Subscribed to channel")
	...
	for {
		...
		case brokerMsg, open := <-sub.MessagesChannel():
			...
			if !open && brokerMsg.IsUninitialized() {
				...
				return ListExchangesResponseMessage{}, extensions.ErrSubscriptionCanceled
			}
			...
			return newListExchangesResponseMessageFromBrokerMessage(brokerMsg)
		case <-ctx.Done(): // Set corrsponding error if context is done
			c.logger.Error(ctx, "Context done before getting message")
			return ListExchangesResponseMessage{}, extensions.ErrContextCanceled
		}
	}
}
